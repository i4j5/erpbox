    ...
    <div>
        <div class="max-w-3xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('document-templates.update', $documentTemplate->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <a href="{{ route('document-templates.show', $documentTemplate->id) }}" class="text-blue-600 hover:text-blue-900 mb-2 mr-2">Скачать шаблон</a> 
                        </div>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            @foreach($allVariables  as $variable)
                                <label class="flex items-center">
                                    <input {{ in_array($variable->id, old('variables', $variables)) ? ' checked' : '' }} name="variables[]" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" wire:model.defer="createApiTokenForm.permissions" value="{{$variable->id}}">
